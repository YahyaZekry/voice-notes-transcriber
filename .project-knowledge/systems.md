...
| Speaker diarization | Live | pyannote `speaker-diarization-3.1` (gated models, needs `HF_TOKEN`). Audio decoded to 16 kHz mono via PyAV (bypasses broken torchcodec) and fed as a waveform dict. Auto speaker count; labels added only when >1 distinct voice. Falls back to plain transcript on any failure. |
